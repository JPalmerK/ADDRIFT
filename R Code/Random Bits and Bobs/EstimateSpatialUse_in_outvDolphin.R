...
for(ii in 1:20){
  ...
  sim.set$SL =rbinom(n = nrow(sim.set),size = 1, prob = .2)*
    rnorm(n = nrow(sim.set), mean = 151, sd = 6)
...
# Transmission loss
alpha = AcousticAbsorption(8000)
TL<- function(r, h=100, alpha=0.7632995){
...
for(ii in 1:nrow(sim.calls)){
  ...
  for(drift in DriftNames){
    ...
    NL_FUN <-  approxfun(NLsub$datetime_posix, NLsub$TOL_6300)
  ...
  whaleGrid$ExpectedSNR_havers = 151-TL(whaleGrid$rElipsoid)-
