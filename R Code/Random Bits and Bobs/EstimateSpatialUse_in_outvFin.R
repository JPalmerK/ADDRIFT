...
for(ii in 1:20){
  ...
  sim.set$SL =rbinom(n = nrow(sim.set),size = 1, prob = .2)*
    runif(n = nrow(sim.set), min = 160, max = 186)
...
# Transmission loss
alpha = AcousticAbsorption(20)
TL<- function(r, h=20, alpha=4.965065e-05){
...
for(ii in 1:nrow(sim.calls)){
  ...
  whaleGrid$ExpectedSNR_havers = 173-TL(whaleGrid$rElipsoid)-
    whaleGrid$NoiseLevel
  
  # For detected calls
  whaleGrid$SNRok_havers[whaleGrid$Detected==1]=
    ifelse(whaleGrid$ExpectedSNR_havers[whaleGrid$Detected==1]+13  >= 
             whaleGrid$SNRHavers[whaleGrid$Detected==1] &
             (whaleGrid$ExpectedSNR_havers[whaleGrid$Detected==1]-13) <= 
